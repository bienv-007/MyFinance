...
<head>
    ...
    <style>
        ...
        .dropdown-panel{position:absolute;right:0;z-index:50;margin-top:0.25rem;width:12rem;transform-origin:top right;border-radius:1rem;border:1px solid #f1f5f9;background:#fff;padding:0.375rem;box-shadow:0 10px 25px -5px rgba(0,0,0,.1),0 8px 10px -6px rgba(0,0,0,.1);transition:opacity .15s ease,transform .15s ease}
        .dropdown-panel-enter{opacity:0;transform:scale(.95) translateY(-4px)}
        .dropdown-item{display:flex;align-items:center;gap:0.75rem;padding:0.625rem 1rem;border-radius:0.75rem;font-size:0.875rem;font-weight:500;transition:background .1s;cursor:pointer;text-decoration:none;color:#334155;width:100%}
        .dropdown-item:hover{background:#f8fafc}
        .dropdown-item.danger{color:#e11d48}
        .dropdown-item.danger:hover{background:#fff1f2}
        .dropdown-separator{margin:0.25rem 0;border-top:1px solid #f1f5f9}
    </style>
</head>
                ...
                <div v-else class="space-y-8">
                                        ...
                                        <tr v-for="item in categories.items" :key="item.id_categorie" class="border-t">
                                            ...
                                            <td class="py-4 text-right">
                                                <div class="relative inline-flex" @click.stop>
                                                    <button @click.stop="toggleDropdown('cat-'+item.id_categorie)" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"><i class="fa-solid fa-ellipsis-vertical text-sm"></i></button>
                                                    <div v-show="openDropdown === 'cat-'+item.id_categorie" class="dropdown-panel">
                                                        <button @click="editCategory(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-pen-to-square w-4 text-center text-xs"></i> Modifier</button>
                                                        <div class="dropdown-separator"></div>
                                                        <button @click="destroy('categories', item.id_categorie); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-trash-can w-4 text-center text-xs"></i> Supprimer</button>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody>
                                    <tr v-for="item in revenus.items" :key="item.id_revenu" class="border-t">
                                        ...
                                        <td class="text-right">
                                            <div class="relative inline-flex" @click.stop>
                                                <button @click.stop="toggleDropdown('rev-'+item.id_revenu)" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"><i class="fa-solid fa-ellipsis-vertical text-sm"></i></button>
                                                <div v-show="openDropdown === 'rev-'+item.id_revenu" class="dropdown-panel">
                                                    <button @click="editRevenu(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-pen-to-square w-4 text-center text-xs"></i> Modifier</button>
                                                    <div class="dropdown-separator"></div>
                                                    <button @click="destroy('revenus', item.id_revenu); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-trash-can w-4 text-center text-xs"></i> Supprimer</button>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <div v-if="activeTab==='previsions-revenus'" class="space-y-6">
                        ...
                        <div class="grid gap-6 lg:grid-cols-3">
                            ...
                            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6 lg:col-span-2">
                                ...
                                <div v-else class="mt-5 space-y-3"><div v-for="item in revenuPrevisions.items" :key="'mobile-' + item.id_revenu_prevision" class="rounded-2xl border border-slate-100 p-4 md:hidden"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><p class="truncate font-semibold text-slate-900">@{{ item.source_previsionnelle }}</p><p class="mt-1 text-sm text-slate-500">@{{ formatDate(item.date_previsionnelle) }}</p></div><span class="shrink-0 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold" :class="revenuPrevisionStatusClass(item.statut)">@{{ item.statut }}</span></div><p class="mt-3 font-semibold text-slate-700">@{{ formatMoney(item.montant_previsionnel) }} <span class="text-xs text-slate-400">FC</span></p><div class="mt-4 flex items-center justify-between"><div class="flex flex-wrap gap-x-4 gap-y-2 text-sm"></div><div class="relative inline-flex" @click.stop><button @click.stop="toggleDropdown('rvp-'+item.id_revenu_prevision)" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"><i class="fa-solid fa-ellipsis-vertical text-sm"></i></button><div v-show="openDropdown === 'rvp-'+item.id_revenu_prevision" class="dropdown-panel"><button v-if="item.statut !== 'Réalisée'" @click="markRevenuPrevision(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-hand-holding-dollar w-4 text-center text-xs"></i> Marquer reçu</button><button @click="editRevenuPrevision(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-pen-to-square w-4 text-center text-xs"></i> Modifier</button><div class="dropdown-separator"></div><button @click="destroy('revenu-previsions', item.id_revenu_prevision); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-trash-can w-4 text-center text-xs"></i> Supprimer</button></div></div></div></div><div class="hidden overflow-x-auto md:block"><table class="w-full min-w-[760px] text-sm"><thead class="border-b border-slate-100 text-left text-xs uppercase tracking-wider text-slate-400"><tr><th class="py-3">Source</th><th class="py-3">Montant</th><th class="py-3">Date</th><th class="py-3">Statut</th><th></th></tr></thead><tbody><tr v-for="item in revenuPrevisions.items" :key="item.id_revenu_prevision" class="border-b border-slate-100 last:border-0"><td class="py-4 font-semibold text-slate-900">@{{ item.source_previsionnelle }}</td><td class="py-4 font-semibold text-slate-700">@{{ formatMoney(item.montant_previsionnel) }} <span class="text-xs text-slate-400">FC</span></td><td class="py-4 text-slate-500">@{{ formatDate(item.date_previsionnelle) }}</td><td class="py-4"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold" :class="revenuPrevisionStatusClass(item.statut)">@{{ item.statut }}</span></td><td class="py-4 text-right whitespace-nowrap"><div class="relative inline-flex" @click.stop><button @click.stop="toggleDropdown('rvpd-'+item.id_revenu_prevision)" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"><i class="fa-solid fa-ellipsis-vertical text-sm"></i></button><div v-show="openDropdown === 'rvpd-'+item.id_revenu_prevision" class="dropdown-panel"><button v-if="item.statut !== 'Réalisée'" @click="markRevenuPrevision(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-hand-holding-dollar w-4 text-center text-xs"></i> Marquer reçu</button><button @click="editRevenuPrevision(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-pen-to-square w-4 text-center text-xs"></i> Modifier</button><div class="dropdown-separator"></div><button @click="destroy('revenu-previsions', item.id_revenu_prevision); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-trash-can w-4 text-center text-xs"></i> Supprimer</button></div></div></td></tr></tbody></table></div></div>
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody>
                                    <tr v-for="item in depenses.items" :key="item.id_depense" class="border-t">
                                        ...
                                        <td class="text-right">
                                            <div class="relative inline-flex" @click.stop>
                                                <button @click.stop="toggleDropdown('dep-'+item.id_depense)" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"><i class="fa-solid fa-ellipsis-vertical text-sm"></i></button>
                                                <div v-show="openDropdown === 'dep-'+item.id_depense" class="dropdown-panel">
                                                    <button @click="editDepense(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-pen-to-square w-4 text-center text-xs"></i> Modifier</button>
                                                    <div class="dropdown-separator"></div>
                                                    <button @click="destroy('depenses', item.id_depense); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-trash-can w-4 text-center text-xs"></i> Supprimer</button>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <div v-if="activeTab==='previsions'" class="space-y-6">
                        ...
                        <div class="grid gap-6 lg:grid-cols-3">
                            ...
                            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6 lg:col-span-2">
                                ...
                                <div v-else class="mt-5 space-y-3"><div v-for="item in previsions.items" :key="'mobile-' + item.id_depense_prevision" class="rounded-2xl border border-slate-100 p-4 md:hidden"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><p class="truncate font-semibold text-slate-900">@{{ item.categorie ? item.categorie.nom_categorie : '-' }}</p><p class="mt-1 text-sm text-slate-500">@{{ formatDate(item.date_previsionnelle) }}</p></div><span class="shrink-0 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold" :class="previsionStatusClass(item.statut)">@{{ item.statut }}</span></div><p class="mt-3 font-semibold text-slate-700">@{{ formatMoney(item.montant_previsionnel) }} <span class="text-xs text-slate-400">FC</span></p><div class="mt-4 flex items-center justify-between"><div class="flex flex-wrap gap-x-4 gap-y-2 text-sm"></div><div class="relative inline-flex" @click.stop><button @click.stop="toggleDropdown('dp-'+item.id_depense_prevision)" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"><i class="fa-solid fa-ellipsis-vertical text-sm"></i></button><div v-show="openDropdown === 'dp-'+item.id_depense_prevision" class="dropdown-panel"><button @click="editPrevision(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-pen-to-square w-4 text-center text-xs"></i> Modifier</button><button @click="validatePrevision(item); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-check w-4 text-center text-xs"></i> Valider</button><div class="dropdown-separator"></div><button @click="destroy('depense-previsions', item.id_depense_prevision); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-trash-can w-4 text-center text-xs"></i> Supprimer</button></div></div></div></div><div class="hidden overflow-x-auto md:block"><table class="w-full min-w-[720px] text-sm"><thead class="border-b border-slate-100 text-left text-xs uppercase tracking-wider text-slate-400"><tr><th class="py-3">Catégorie</th><th class="py-3">Montant</th><th class="py-3">Date</th><th class="py-3">Statut</th><th></th></tr></thead><tbody><tr v-for="item in previsions.items" :key="item.id_depense_prevision" class="border-b border-slate-100 last:border-0"><td class="py-4 font-semibold text-slate-900">@{{ item.categorie ? item.categorie.nom_categorie : '-' }}</td><td class="py-4 font-semibold text-slate-700">@{{ formatMoney(item.montant_previsionnel) }} <span class="text-xs text-slate-400">FC</span></td><td class="py-4 text-slate-500">@{{ formatDate(item.date_previsionnelle) }}</td><td class="py-4"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold" :class="previsionStatusClass(item.statut)">@{{ item.statut }}</span></td><td class="py-4 text-right whitespace-nowrap"><div class="relative inline-flex" @click.stop><button @click.stop="toggleDropdown('dpd-'+item.id_depense_prevision)" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"><i class="fa-solid fa-ellipsis-vertical text-sm"></i></button><div v-show="openDropdown === 'dpd-'+item.id_depense_prevision" class="dropdown-panel"><button @click="editPrevision(item); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-pen-to-square w-4 text-center text-xs"></i> Modifier</button><button @click="validatePrevision(item); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-check w-4 text-center text-xs"></i> Valider</button><div class="dropdown-separator"></div><button @click="destroy('depense-previsions', item.id_depense_prevision); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-trash-can w-4 text-center text-xs"></i> Supprimer</button></div></div></td></tr></tbody></table></div></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="activeTab==='notifications'" class="mx-auto max-w-4xl space-y-6">
                        ...
                        <article v-for="notification in notifications.items" :key="notification.id_notification" class="flex gap-4 rounded-3xl border p-5 shadow-sm" :class="notification.est_lue ? 'border-slate-200 bg-white opacity-75' : 'border-indigo-300 bg-indigo-50 ring-1 ring-indigo-100'">
                            ...
                            <div class="relative inline-flex" @click.stop>
                                <button @click.stop="toggleDropdown('notif-'+notification.id_notification)" class="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"><i class="fa-solid fa-ellipsis-vertical text-sm"></i></button>
                                <div v-show="openDropdown === 'notif-'+notification.id_notification" class="dropdown-panel">
                                    <button v-if="!notification.est_lue" @click="markNotificationAsRead(notification); openDropdown = null" class="dropdown-item"><i class="fa-solid fa-check w-4 text-center text-xs"></i> Marquer comme lue</button>
                                    <div class="dropdown-separator"></div>
                                    <button @click="deleteNotification(notification); openDropdown = null" class="dropdown-item danger"><i class="fa-solid fa-trash-can w-4 text-center text-xs"></i> Supprimer</button>
                                </div>
                            </div>
                        </article>
                    </div>
